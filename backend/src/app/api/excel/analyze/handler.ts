...
function logWithTimestamp(message: string) {
  ...
  console.log(`[${timestamp}] 🔄 BACKEND EXCEL HANDLER - ${message}`);
}

export interface ExcelAnalysisParams {
  file_path: string;
  model: string;
  use_tools: boolean;
  auth_token: string;
...
export async function handleExcelAnalysis(params: ExcelAnalysisParams) {
  ...
  try {
    ...
    logWithTimestamp(`Error in Excel analysis: ${error instanceof Error ? error.message : 'Unknown error'}`);
    throw error;
  }
} 