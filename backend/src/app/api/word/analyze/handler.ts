...
import { callWordLLMAPI } from '@/lib/llm-api';
...
function logWithTimestamp(message: string) {
  ...
  console.log(`[${timestamp}] 🔄 BACKEND WORD HANDLER - ${message}`);
}

export interface WordAnalysisParams {
  file_path: string;
  model: string;
  use_tools: boolean;
  auth_token: string;
...
export async function handleWordAnalysis(params: WordAnalysisParams) {
  ...
  try {
    ...
    logWithTimestamp(`Error in Word analysis: ${error instanceof Error ? error.message : 'Unknown error'}`);
    throw error;
  }
} 