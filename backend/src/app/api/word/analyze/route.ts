...
import { handleWordAnalysis } from './handler';

// Enhanced logging function
function logWithTimestamp(message: string) {
  const now = new Date();
  const timestamp = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}:${now.getSeconds().toString().padStart(2, '0')}.${now.getMilliseconds().toString().padStart(3, '0')}`;
  console.log(`[${timestamp}] 🔄 BACKEND WORD API - ${message}`);
}
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      // Call the handler function to analyze the Word file
      logWithTimestamp(`Calling handler function for analysis`);
      const analysisResult = await handleWordAnalysis({
        file_path,
        model,
        use_tools,
        auth_token: token,
